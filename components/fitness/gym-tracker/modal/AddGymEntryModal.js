...
const AddGymEntryModal = () => {
    ...
    const [exerciseTypeId, setExerciseType] = useState(0);
    ...
    const onHandleExerciseTypeChange = (exerciseTypeId) => { setExerciseType(exerciseTypeId) }
    ...
    const onSubmit = (event) => {
        ...
        ExerciseService.addExerciseInformation(data)
            .then((response) => {
                 setExerciseName(response.exerciseName);
                 setDate(response.date);
                 setSets(response.sets);
                 setReps(response.reps);
                 setWeight(response.weight);
                 setExerciseType(response.exerciseTypeId);
